...
4. Host memilih blanko `2`/`3`/`4`/`6`, `Classic`/`Youth`/`Mono`, timer, toggle Otomatis (default mati), efek kamera, overlay, dan latar virtual. Tamu memakai pilihan host (termasuk gambar latar unggahan host yang disinkronkan). Progress mengikuti `Format` / `Foto` / `Review` / `Hasil`.
