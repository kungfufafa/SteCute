...
2. Host menekan `Buat Booth` lalu mengatur Jumlah Foto, Frame, Timer, dan Otomatis di Atur Sesi, sama dengan `Mulai Foto` (tanpa upload frame custom).
...
- Host mengatur layout bundled `2`/`3`/`4`/`6`, template `Classic`/`Youth`/`Mono`, countdown (default `3` detik), dan toggle Otomatis (default mati; pose pertama tetap diklik, pose berikutnya auto countdown seperti `Mulai Foto`) di Atur Sesi sebelum masuk ruang. Di ruang live, host memilih preset `Efek Kamera`, `Overlay Kamera`, dan `Latar Virtual` yang sama dengan alur kamera lokal. Jumlah momen capture sama dengan jumlah slot layout, bukan strip 2-pose Classic yang dikunci. Warna studio dan blur mengikuti setup host di kedua tile; gambar unggahan hanya di perangkat yang punya file.
